...
function topSearchWid(){
    if($('.main-back').length>0){
        var searchWid = Math.ceil($('.main-title').width())-Math.ceil($('.main-back').width())-85;
        // 搜索框宽度
        // 搜索页面搜索关键词宽度0
        ...
        var searchWid = Math.ceil($('.main-title').width())-Math.ceil($('.main-logo').width())-Math.ceil(riBtnwid)-40;
        var inputWid = searchWid;
    }
...
$('#menu-mask2').click(function(){//右上角
    $('#detail-top-menu').removeClass('show');
})

$('.main-title').click(function(e){
    if(e.target.id == "mui-action-menu" || e.target.id == "menu-cat-btn" || e.target.parentNode.id == 'menu-cat-btn'){
        $('#menu-mask2').click();
        ...
        if($('#detail-top-menu').hasClass('show')){
            ...
            if(e.target.id == "cat-action-menu"){
